...
```{r}
# sim <- "sim_og"
# dat <- readRDS("./data/gpkg/2.1_acChoiceSet_rumsimog_4283.gpkg")  # this is the generalised data with static site attributes
# b <- read_excel("./data/03_data/ac_sim_og.xlsx") %>% dplyr::select(Vars, Coef)
# v <- read_excel("./data/03_data/ac_sim_og.xlsx", sheet = "v", col_names = rep("x", 7)) %>%
#   dplyr::select(-x...1 ) %>%
#   distinct()
#   
# sim <- "sim1"
# dat <- readRDS("./data/gpkg/2.1_acChoiceSet_rumsim1_4283.gpkg")  # this is the generalised data with static site attributes
# b <- read_excel("./data/03_data/ac_sim1.xlsx") %>% dplyr::select(Vars, Coef)
# v <- read_excel("./data/03_data/ac_sim1.xlsx", sheet = "v", col_names = rep("x", 7)) %>%
#   dplyr::select(-x...1 ) %>%
#   distinct()
#   
# sim <- "sim2"
# b <- read_excel("./data/03_data/ac_sim2.xlsx") %>% dplyr::select(Vars, Coef)
# v <- read_excel("./data/03_data/ac_sim2.xlsx", sheet = "v", col_names = rep("x", 7)) %>% 
#   dplyr::select(-x...1 ) %>% 
#   distinct()
#   

# sim <- "sim3"
# b <- read_excel("./data/03_data/ac_sim3.xlsx") %>% dplyr::select(Vars, Coef)
# v <- read_excel("./data/03_data/ac_sim3.xlsx", sheet = "v", col_names = rep("x", 6)) %>%
#   dplyr::select(-x...1 ) %>%
#   distinct()
#   
# sim <- "sim4"
# dat <- readRDS("./data/gpkg/2.1_acChoiceSet_rumsim4_4283.gpkg")  # this is the generalised data with static site attributes
# b <- read_excel("./data/03_data/ac_sim4.xlsx") %>% dplyr::select(Vars, Coef)
# v <- read_excel("./data/03_data/ac_sim4.xlsx", sheet = "v", col_names = rep("x", 7)) %>%
#   dplyr::select(-x...1 ) %>%
#   distinct()


# sim <- "simog_split"
# b <- read_excel("./data/03_data/ac_simog_split.xlsx") %>% dplyr::select(Vars, Coef)
# v <- read_excel("./data/03_data/ac_simog_split.xlsx", sheet = "v", col_names = rep("x", 10)) %>%
#   dplyr::select(-x...1 ) %>%
#   distinct()
#   
# sim <- "sim1_split"
# dat <- readRDS("./data/gpkg/2.1_acChoiceSet_rumsim1_split_4283.gpkg")  # this is the generalised data with static site attributes
# b <- read_excel("./data/03_data/ac_sim1_split.xlsx") %>% dplyr::select(Vars, Coef)
# v <- read_excel("./data/03_data/ac_sim1_split.xlsx", sheet = "v", col_names = rep("x", 10)) %>%
#   dplyr::select(-x...1 ) %>%
#   distinct()

# sim <- "sim4_split"
# dat <- readRDS("./data/gpkg/2.1_acChoiceSet_rumsim4_split_4283.gpkg")  # this is the generalised data with static site attributes
# b <- read_excel("./data/03_data/ac_sim4_split.xlsx") %>% dplyr::select(Vars, Coef)
# v <- read_excel("./data/03_data/ac_sim4_split.xlsx", sheet = "v", col_names = rep("x", 10)) %>%
#   dplyr::select(-x...1 ) %>%
#   distinct()
#   
...
v <- read_excel("./data/03_data/aclogit_sim4_split.xlsx", sheet = "v", col_names = rep("x", 38)) 
  # dplyr::select(-x...1 ) %>%
  # distinct()
```
...
```{r rearrange, include = FALSE}
    ...
    b %<>% rename(gridid_alt = ...1) %>% 
      mutate(gridid_alt = ifelse(gridid_alt %in% "gridid_alt", 1, gridid_alt)) %>% 
      dplyr::select(-Vars)
  
    v %<>% mutate(x...1 = ifelse(x...1 %in% "gridid_alt", 1, x...1)) %>% 
      dplyr::select(-x...2) %>% 
      distinct()
...
# define vars used in RUM
  # sz_vars <- dat %>% dplyr::select(num_range("sz", 1:100))
  # vars <- c("travelCost", names(sz_vars))
  vars <- c("travelCost")


  # vars <- unique(dat$gridID_alt)
    
# turn defined vars into matrix from dat, make sure there is no geometry
    rum_matrix <- as.matrix(dat[, vars])
   
    rum_matrix <- dat %>% dplyr::select(travelCost, gridID_alt)
    # 
     rum_matrix <- pivot_wider(rum_matrix, names_from = gridID_alt, values_from = travelCost)
    # 
    # rum_matrix %<>% pivot_longer(cols = 1:2, names_to = "gridID_alt", values_to = "travelCost")
    # 
    rum_matrix %<>% unnest(1:length(rum_matrix)) %>%
    # distinct() %>%
    as.matrix()
      
    # rum_matrix <- as.matrix(unique(dat$gridID_alt))
