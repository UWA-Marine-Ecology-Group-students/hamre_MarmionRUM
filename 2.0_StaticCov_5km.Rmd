...
BR <- st_read("./data/gpkg/BR_4283.gpkg") %>%  st_transform(4283)# boat ramps
coast <- st_read("./data/gpkg/Coast_4283.gpkg") %>% st_transform(4283)# coast
# habitat <- st_read("./data/gpkg/espHabitat_4283.gpkg") # habitat
sites <- st_read("./data/gpkg/mar5kmgrid.gpkg") %>%  st_transform(4283)# MVM grid
studySite <- st_read("./data/gpkg/StudySite_4283.gpkg") %>% st_transform(4283) # study site (mega mvm polygon)
# artInfra <- st_read("./data/gpkg/espArtInfra_4283.gpkg")
shelter <- st_read("./data/gpkg/mar5km_shelter.gpkg") %>% st_transform(4283)
NPZ <- st_read("./data/gpkg/ntz_4283.gpkg") %>% st_transform(4283)# commonwealth and current state NTZ
...
```{r}
sites$shelter <- ifelse(sites$fid %in% shelter$fid, 1, 0)
...
waterGrid <- waterGrid %>% 
  mutate(commhex = ifelse(as.numeric(area) >= 22, 1, 0))
...
#   
ggplot() +
  geom_sf(data = waterGrid, aes(fill = commhex))
```
...
## Save output
```{r save}
saveRDS(waterGrid, "./data/gpkg/2.0_5kmSites_4283.gpkg")
```