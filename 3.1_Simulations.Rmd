...
# data
dat <- readRDS("./data/gpkg/2.1_scbioChoiceSet_4283.gpkg") # this is the generalised data with static site attributes
BRtrips <- readRDS("./data/gpkg/2.1_scBRtrips.gpkg") # This is produced by the "TripAllocation.R" script (Matt)

# Data with coefficients from model using trip data
b <- read_excel("./ignore/03_data/mod.xlsx") %>% dplyr::select(Vars, Coef)
 
# covariance matrix
v <- read_excel("./ignore/03_data/mod.xlsx", sheet = "v", col_names = rep("x", 8)) %>% 
  dplyr::select(-x...1 ) %>% 
  distinct()

# Plotting
mp <- st_read("./data/gpkg/pscmpPort_4326.gpkg")
mp_bbox <- st_read("./data/gpkg/pscmpBbox_4326.gpkg")
NPZ <- st_read("./data/gpkg/scCommNP_4283.gpkg")
...
dat <- dat %>% 
  ...
  mutate(arealog = ifelse(is.infinite(arealog), 0, arealog)) %>% 
  st_drop_geometry() %>%
  as.data.frame()
```
...
# For each site p has 100 estimates of the p of visiting that site me give me tyhe average, median, ci..
    ...
    dat$p.mean_base <- rowQuantiles(p, probs = 0.5) # median
...
# Assign closures
sim4 <- st_read("./data/gpkg/sims/scSim4_4283.gpkg")

    # sims <- list.files("./data/gpkg/sims/") 

ntz <- c(sim4$gridID_alt)
...
# Calculates probability of visit summary statistics by row. This is the simulated probability of each site, for each trip and the error. 
    ...
    dat$p.mean_sim <- rowQuantiles(p, probs = 0.5)
...
# Summaries across draws
    W <- as.matrix(W)
    logsum_sim$w.mean <- rowQuantiles(W, probs = 0.5)
    ...
    W_impact_trip <- logsum_sim %>%
      summarise("Annual mean welfare impact per trip ($)" = round(sum(w.mean), 2),
                "Annual upper welfare impact per trip ($)" = round(sum(w.low), 2),
                "Annual lower welfare impact per trip ($)" = round(sum(w.upp), 2))

    W_impact_trip
    png("./plots/sim4.W_impact_trip.png", height = 50*nrow(W_impact_trip), width = 200*ncol(W_impact_trip))
...
# Sum by grid (over ramps) to get use at grid level
      RUM_grid <- RUM %>% group_by(gridID_alt, BR, Ramp_name) %>%  # was just grouping by ID
...
W_impact <- RUM_W %>% summarise(round(across(starts_with(c("welf_mean","welf_upp","welf_low")), sum)))

W_impact %<>%  rename("Annual mean welfare impact ($)" = welf_mean_TRIPS_ramp_prop_Grvt__5,
                        "Annual upper welfare impact ($)" = welf_upp_upp_TRIPS_Grvt__5,
                        "Annual lower welfare impact ($)" = welf_low_low_TRIPS_Grvt__5)

W_impact
png("./plots/sim4.W_impact.png", height = 50*nrow(W_impact), width = 200*ncol(W_impact))
grid.table(W_impact)
dev.off()
```
...
ggsave(path = "./plots", filename = paste("sim4.presim.png", sep = ''), plot = presim, width = 8, height = 4)
...
ggsave(path = "./plots", filename = paste("sim4.postsim.png", sep = ''), width = 8, height = 4)
...
W_BRimpact <- RUM_W %>% group_by(BR, Ramp_name) %>%
      summarise(round(across(starts_with(c("welf_mean","welf_upp","welf_low")), sum))) %>% 
...
W_BRimpact_bar
ggsave(path = "./plots", filename = paste("sim4.W_BRimpact_bar.png", sep = ''), plot = W_BRimpact_bar, width = 8, height = 4)


W_BRimpact %<>%  rename("Annual mean welfare impact ($)" = welf_mean_TRIPS_ramp_prop_Grvt__5,
                        "Annual upper welfare impact ($)" = welf_upp_upp_TRIPS_Grvt__5,
                        "Annual lower welfare impact ($)" = welf_low_low_TRIPS_Grvt__5)

W_BRimpact
png("./plots/sim4.W_BRimpact.png", height = 50*nrow(W_BRimpact), width = 150*ncol(W_BRimpact))
...
ggsave(path = "./plots", filename = paste("sim4.", br$Ramp_name,"_presim.png", sep = ''), plot = presim, width = 8, height = 4)
print(presim)
...
ggsave(path = "./plots", filename = paste("sim4.", br$Ramp_name,"_postsim.png", sep = ''), plot = postsim, width = 8, height = 4)
print(postsim)
}
```
...
ggsave(path = "./plots", filename = paste("sim4.", br$Ramp_name,"_diff.png", sep = ''), plot = diff, width = 8, height = 4)
print(diff)
}
```