...
```{r}
state_line <- st_intersection(comm, state) %>% # exatrcting lin in common between comm and state
...
vert_split_holes_df <- vert_split_holes %>% as.data.frame() %>% mutate(geom = as.character(geom))
ntz_df <- ntz %>% as.data.frame() %>% mutate(geometry = as.character(geom))
...
# ntz_line <- st_cast(ntz, 'MULTILINESTRING') # change to lines
# ntz_split <- st_split(vert_split, ntz_line) # split vert split
# ntz_split <- st_collection_extract(ntz_split) 
# 
# a <- ntz_split %>% mutate(id = row_number()) %>% distinct()

# ntz_split_lines <- st_cast(ntz_split, "MULTILINESTRING")

ggplot() +
  # geom_sf(data = sim) +
  # geom_sf(data = sim_holes, aes(fill = as.factor(id)))
  # geom_sf(data = sim_holes_id2, fill = 'red')
  # geom_sf(data = sim_holes, fill = 'red')
  geom_sf(data = vert_split_holes, fill = "red")
```
