...
#
# TODO
# fix loose ends of state polygon
# merge small polygons, not filter
# let the function append the npz and the sims (separate function?)
# let function do all polygons within vertical bounds 
# vertical lines are one long polygon from top to bottom
# do for all rows in poly, get info for both polygons not just last one
...
ASCgrid <- function(poly, #  sf polygon to split
                    ...
                    vert = FALSE, # vertical or horizontal divisions
                    crs = 4283) { # makes all crs the same, output crs
  ...
  # Preps data for loop
  poly_w_ntz <- st_difference(row, st_combine(ntz)) 
...
t <- ASCgrid(poly = vert_split, ntz = ntz, point = dat, from = 0.03, by = 0.02, min_area = 30, crs = 4283, vert = FALSE)

```


```{r}
t <- ASCgrid(poly = state, ntz = ntz, point = dat, from = 0.03, by = 0.02, min_area = 30)

t2 <- ASCgrid(poly = comm, ntz = ntz, point = dat)

t <- ASCgrid(poly = state_holes, point = dat)

s <- ASCgrid(poly = state, ntz = ntz, point = dat, from = 0.03, by = 0.02, min_area = 30, vert = TRUE)

ggplot() +
  # geom_sf(data = a, aes(fill = as.factor(id))) +
  # geom_sf(data = dat) +
   # geom_sf(data = a, fill = "red")+
   geom_sf(data = t, fill = "red")

   # geom_sf(data = bbox) +
  # geom_sf(data = state_holes)
  # geom_sf(data = t, fill = "red")+
  # geom_sf(data = t2, fill = "blue") +
  # geom_sf(data = ntz, fill = "pink")
...
#   
#   for (i in hGrid) { 
#     grid <- poly_w_ntz %>% st_make_grid(cellsize = c(i, i))
#     grid <- st_intersection(poly_w_ntz, grid)
#     grid$area <- as.numeric(round(set_units(st_area(grid), km^2), 2))
#     grid %<>% filter(area > min_area)
#     temp <- as.data.frame(st_intersects(grid, point, sparse = FALSE)) %>%
...
# }
# 
# t <- ASCgrid(poly = state, ntz = ntz, point = dat, from = 0.003, by = 0.02, min_area = 30)
