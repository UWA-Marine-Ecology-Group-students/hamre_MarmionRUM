...
ASCgrid <- function(poly, #  sf polygon to split
  ...
  if (class(poly) != "sf" || class(ntz) != "sf" || class(point) != "sf" ) {
    stop("Poly is not an sf object. Change format to sf, see sf::st_as_sf()")
  }
    ...
    temp <- as.data.frame(st_intersects(grid, point, sparse = FALSE)) %>%
      ...
      if (any(grid$data_present == FALSE)) {
        ...
        print(
          ...
          geom_sf(data = grid, alpha = 0, lwd = 0.5)
        )
        return(grid)
        break
      }
...
demo <- ASCgrid(poly = state, ntz = ntz, point = dat, from = 0.03, by = 0.02, min_area = 30, crs = 4283, vert = TRUE) # crs using degrees
```
...
a <- as.data.frame(state)

asc_test(poly = a, ntz = ntz, point = dat, from = 0.03, by = 0.03, min_area = 30, crs = 4283, vert = FALSE) # crs using degrees
asc_test(poly = state, ntz = ntz, point = dat, from = 1, by = 2, min_area = 30, crs = 28350, vert = FALSE, units = "km") 
...
st_crs(a, parameters = TRUE)$units_gdal
