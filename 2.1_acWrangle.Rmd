...
asc_test <- function(poly, #  sf polygon to split
  ...
  full_grid <- cbind(full_grid, full_geom) %>%
   ...
   print(
     ggplot() +
       geom_sf(data = full_grid, aes(fill = as.factor(sz_wo_value)), lwd = 0.5) +
       geom_sf(data = point, size = 0.5)
   )
   return(full_grid)
}
...
# mergeing sz data to grid 
 # joining sims and npz while maintaining attributes

  acGrid_geom <- st_as_sf(acGrid$geom)
  ntz_geom <- st_as_sf(ntz$geom) 
  full_geom <- rbind(acGrid_geom, ntz_geom)

  acGrid  %<>% as.data.frame() %>% mutate(geom = as.character(geom))
  ntz %<>% as.data.frame() %>% mutate(geom = as.character(geom))

  full_grid <- full_join(acGrid, ntz) %>%
    mutate(id = row_number()) %>%
    mutate_if(is.numeric, ~replace_na(., 0))
...
ggplot() +
  geom_sf(data = full_grid, aes(fill = as.factor(sz_wo_value)))
```
