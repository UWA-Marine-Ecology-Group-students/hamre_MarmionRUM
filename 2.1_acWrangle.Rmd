...
# function started for attributes
```{r}
...
# 
ggplot()+
  geom_sf(data = dat_rum_sf$geom, aes(fill = as.factor(dat_rum_sf$gridID_alt)))

a <- dat_rum_sf %>%  filter(gridID_vis %in% c(41))

ggplot()+
  geom_sf(data = a$geom, aes(fill = as.factor(a$gridID_vis)))
```
