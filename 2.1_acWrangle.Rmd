...
asc_test(poly = vert_split, ntz = ntz, point = dat, from = 0.03, by = 0.02, min_area = 0, crs = 4283, vert = FALSE) # crs using degrees
...
state <- st_transform(state, crs(ntz))
  ...
  state_hole <- st_difference(st_make_valid(state), st_combine(ntz)) # crops ntz out of grid
  
  state_hole %<>% 
    st_cast('MULTIPOLYGON') %>%
    st_cast("POLYGON") 

  state_hole$area <- as.numeric(round(set_units(st_area(state_hole), km^2), 2))
  a <- state_hole %>% filter(area > 1) %>% mutate(id = row_numbe)
