...
sites <- readRDS("./data/gpkg/2.0_scbioSites_4283.gpkg") # sites with static attributes
dat <- read_csv("ignore/01_data/1.2_esp.csv") %>% filter(ActivityType == "Extractive")
BR <- st_read("./data/gpkg/scbioBR_4283.gpkg") %>%  dplyr::select(RampID) # boat ramps
...
st_write(choice, "./data/gpkg/2.1_scbioChoiceSetQ_4283.gpkg", append = FALSE) # need to write with st_write to open in QGISRUMs

saveRDS(sites, "./data/gpkg/2.1_scbioChoiceSet_4283.gpkg")
```
...
dat <- spdat %>% 
  ...
  rename(gridID_vis = gridID_alt, BR = Site) %>% 
  filter(ActivityType %in% "Extractive") %>% 
  dplyr::select(TripID, PersonID, gridID_vis, BR, TripDate) %>% 
  distinct() %>% 
  filter(BR %in% c("EsperanceBR", "BandyCreekBR")) %>% 
  mutate(BR = ifelse(BR %in% "EsperanceBR", "65", "66")) %>% 
  mutate(UseLong = st_coordinates(.)[,1]) %>% 
  mutate(UseLat = st_coordinates(.)[,2]) %>% 
  as.data.frame() # %>% 
  # mutate(Line = ifelse(FishingType %in% c("Demersal", "Casting", "Squdding"), 1, 0)) %>% 
  # mutate(Troll = ifelse(FishingType %in% "Trolling", 1, 0)) %>% 
  # mutate(Dive = ifelse(FishingType %in% c("Cray", "Scuba", "Diving", "Spearing"), 1, 0))
...
# Save output
```{r save}
write_csv(dat, "./ignore/02_data/2.1_scbio.csv")
```
...
```{r}
BRtrips <- read.csv("./ignore/02_data/scBRdat.csv") %>% 
...
write_csv(BRtrips, "./ignore/02_data/2.1_scBRtrips.csv")
saveRDS(BRtrips, "./data/gpkg/2.1_scBRtrips.gpkg")
```